...
export const BentoGridItem = ({
  ...
  const leftLists = ["ReactJS", "NextJS", "Typescript", "Zig", "Docker"];
  const rightLists = ["HTMX", "Tailwindcss", "GoLang"];

  const [copied, setCopied] = useState(false);

  const handleCopy = () => {
    const text = "codygibbs432@gmail.com";
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(to bottom, rgba(0, 0, 0, 0.7), transparent)", // Black to transparent gradient
      }}
    >
      <div
        className={`${
          id === 6 && "flex justify-center"
        } bg-black-200 h-full relative`}
      >
...
=======
              className={cn(imgClassName, "object-cover object-center z-0")}
            />
          )}
        </div>
        {/** Add the black gradient above the image for the first item */}
        {(id === 1) && (
          <div
            className="absolute inset-0 bg-gradient-to-tr from-black to-transparent z-10 pointer-events-none"
          ></div>
        )}
        <div
          className={`absolute right-0 -bottom-5 ${
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                ...
                <span className="lg:py-4 lg:px-3 py-4 px-3  rounded-lg text-center bg-green"></span>
              </div>
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                <span className="lg:py-4 lg:px-3 py-4 px-3  rounded-lg text-center bg-gold-100"></span>
                {rightLists.map((item, i) => (
                  <span
                    key={i}
                    className="lg:py-4 lg:px-3 py-2 px-3 text-xs lg:text-base opacity-50 
                    lg:opacity-100 rounded-lg text-center bg-gold"
                  >
                    {item}
                  </span>
                ))}
